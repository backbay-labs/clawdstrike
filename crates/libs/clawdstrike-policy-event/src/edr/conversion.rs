//! PolicyEvent -> Endpoint conversion, content hashing, and metadata field
//! extraction.
//!
//! Bridges the transport-neutral `PolicyEvent` representation into the EDR
//! `EndpointObservation`/`EndpointEvent` model, promotes identity context into
//! metadata, and provides the shared content-hash and field-extractor helpers
//! the rest of the EDR module relies on.
...
use crate::event::{
    CustomEventData, FileEventData, NetworkEventData, PolicyEvent, PolicyEventData, PolicyEventType,
};

use super::{
    credential_kind_from_secret, CausalGraph, EndpointDecisionActor, EndpointEvent,
    EndpointObservation, EndpointProcess, EndpointSensorState, FileOperation,
};

pub(crate) fn canonical_graph_content_hash(graph: &CausalGraph) -> Option<String> {
...
pub(crate) fn endpoint_event_from_policy_event(event: &PolicyEvent) -> EndpointEvent {
...
pub(crate) fn metadata_as_btree(
...
pub(crate) fn process_from_metadata(
    metadata: &BTreeMap<String, serde_json::Value>,
) -> EndpointProcess {
...
pub(crate) fn normalized_identity_value(value: Option<&str>) -> Option<String> {
    value
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .map(ToString::to_string)
...
pub(crate) fn agent_id_field(metadata: &BTreeMap<String, serde_json::Value>) -> Option<String> {
    string_field(
        metadata,
        &[
            "agentId",
...
pub(crate) fn workload_id_field(metadata: &BTreeMap<String, serde_json::Value>) -> Option<String> {
    string_field(
        metadata,
        &[
            "workloadId",
...
pub(crate) fn approval_id_field(metadata: &BTreeMap<String, serde_json::Value>) -> Option<String> {
    string_field(
        metadata,
        &[
            "approvalId",
...
pub(crate) fn tool_call_id_field(metadata: &BTreeMap<String, serde_json::Value>) -> Option<String> {
    string_field(
        metadata,
        &[
            "toolCallId",
...
pub(crate) fn string_field_nested(
    metadata: &BTreeMap<String, serde_json::Value>,
    nested: Option<&serde_json::Map<String, serde_json::Value>>,
    keys: &[&str],
) -> Option<String> {
