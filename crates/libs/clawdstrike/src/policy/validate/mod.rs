...
use checks::{validate_domain_globs, validate_globs, validate_placeholders_in_strings};
use specs::validate_custom_guards;
use version::{policy_version_supports_posture, validate_policy_version};

// Re-export the placeholder validators consumed by the `specs` sibling so they
// stay reachable as `super::validate_placeholders_in_*` from this module.
pub(in crate::policy) use checks::{
    validate_placeholders_in_json, validate_placeholders_in_string,
};
