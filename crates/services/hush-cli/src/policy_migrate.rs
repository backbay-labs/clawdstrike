...
pub(crate) fn migrate_policy_yaml(
    ...
    if is_hushspec_input && !to_hushspec {
        let spec = hushspec::HushSpec::parse(input_yaml).map_err(|e| PolicyMigrateError {
            code: ExitCode::ConfigError,
            kind: "config_error",
            message: format!("Failed to parse HushSpec: {}", e),
        })?;
        let policy =
            clawdstrike::hushspec_compiler::compile(&spec).map_err(|e| PolicyMigrateError {
                code: ExitCode::ConfigError,
                kind: "config_error",
