...
pub fn cmd_init(args: InitArgs, stdout: &mut dyn Write, stderr: &mut dyn Write) -> ExitCode {
    ...
    if do_keygen {
        lines.push(format!("  {} keys/clawdstrike.key", "✓"));
        lines.push(format!("  {} keys/clawdstrike.pub", "✓"));
    }
    lines.push(String::new());
    lines.push("Next steps:".to_string());
    lines.push("  clawdstrike check --action-type file /tmp/test.txt".to_string());
    lines.push("  clawdstrike policy lint .clawdstrike/policy.yaml".to_string());
...
fn write_policy_yaml(path: &Path, ruleset: &str) -> std::io::Result<()> {
    let content = format!(
        r#"version: "1.2.0"
name: "project policy"
description: "Auto-generated by clawdstrike init"
...
# [defaults]
# ruleset = "default"
# signing_key = ".clawdstrike/keys/clawdstrike.key"
...
fn generate_keypair(keys_dir: &Path) -> anyhow::Result<()> {
    ...
    let key_path = keys_dir.join("clawdstrike.key");
    let pub_path = keys_dir.join("clawdstrike.pub");
...
mod tests {
    ...
    fn init_non_interactive_with_keygen() {
        ...
        let cs_dir = tmp.path().join(".clawdstrike");
        assert!(cs_dir.join("keys/clawdstrike.key").exists());
        assert!(cs_dir.join("keys/clawdstrike.pub").exists());
        ...
        #[cfg(unix)]
        {
            ...
            let meta = std::fs::metadata(cs_dir.join("keys/clawdstrike.key")).unwrap();
