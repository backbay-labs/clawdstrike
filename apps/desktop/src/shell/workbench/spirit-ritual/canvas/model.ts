...
function compressSurfaceList(surfaces: string[]): string {
  return surfaces
    .slice(0, 3)
    .join(", ");
}
...
function buildConsequenceLine(candidate: SpiritBindCandidate): string {
  const surfaces = compressSurfaceList(candidate.predictedFocusSurfaces);
...
function describeArtifactLabel(artifactId: string, artifact: Artifact | undefined): string {
  if (!artifact) {
...
function buildGhosts(
  ...
  return candidate.alternates.slice(0, 3).map((alternate, index) => {
    ...
    const position = layouts[index] ?? layouts[layouts.length - 1];
    const contour = meta?.contour ?? "reticle-vector";
    return {
...
export function buildSpiritManifestationModel(
  ...
  const accentColor = runtime.accentColor ?? meta?.accentColor ?? "#d4a84b";
  const contour = runtime.contour ?? meta?.contour ?? "reticle-vector";
  ...
  return {
    ...
    chamberTitle: `${meta?.label ?? candidate.label} manifestation`,
    stationLabel,
    ...
    release: resolveReleaseCue(meta?.label ?? candidate.label, runtime.stance, stationLabel, fieldStrength),
    runtime,
    stage: {
      kind: candidate.kind,
      mode,
