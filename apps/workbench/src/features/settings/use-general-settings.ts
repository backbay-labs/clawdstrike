// General Settings — app-level preferences (theme, editor, autosave)
// Persisted to localStorage independently of the policy store.
//
// Zustand + immer store (migrated from React Context + useState).
import type { ReactElement, ReactNode } from "react";
import { create } from "zustand";
import { immer } from "zustand/middleware/immer";
import { createSelectors } from "@/lib/create-selectors";


export type Theme = "dark";
export type FontSize = "small" | "medium" | "large";
export type AutosaveInterval = "off" | "5" | "15" | "30" | "60";

export interface GeneralSettings {
  theme: Theme;
  fontSize: FontSize;
  autosaveInterval: AutosaveInterval;
  showLineNumbers: boolean;
}

export const DEFAULT_GENERAL_SETTINGS: GeneralSettings = {
  theme: "dark",
  fontSize: "medium",
  autosaveInterval: "30",
  showLineNumbers: true,
};


const STORAGE_KEY = "clawdstrike_general_settings";

function loadSettings(): GeneralSettings {
  try {
    const raw = localStorage.getItem(STORAGE_KEY);
    if (!raw) return DEFAULT_GENERAL_SETTINGS;
    const parsed = JSON.parse(raw);
    if (typeof parsed !== "object" || parsed === null) return DEFAULT_GENERAL_SETTINGS;

    // Validate each field and fall back to defaults for invalid values
    const theme: Theme = "dark"; // only dark supported for now
    const fontSize: FontSize =
      parsed.fontSize === "small" || parsed.fontSize === "medium" || parsed.fontSize === "large"
        ? parsed.fontSize
        : DEFAULT_GENERAL_SETTINGS.fontSize;
    const autosaveInterval: AutosaveInterval =
      parsed.autosaveInterval === "off" ||
      parsed.autosaveInterval === "5" ||
      parsed.autosaveInterval === "15" ||
      parsed.autosaveInterval === "30" ||
      parsed.autosaveInterval === "60"
        ? parsed.autosaveInterval
        : DEFAULT_GENERAL_SETTINGS.autosaveInterval;
    const showLineNumbers: boolean =
      typeof parsed.showLineNumbers === "boolean"
        ? parsed.showLineNumbers
        : DEFAULT_GENERAL_SETTINGS.showLineNumbers;

    return { theme, fontSize, autosaveInterval, showLineNumbers };
  } catch {
    return DEFAULT_GENERAL_SETTINGS;
  }
}

function persistSettings(settings: GeneralSettings): void {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(settings));
  } catch {
    // Storage full or unavailable — ignore
  }
}

// ---------------------------------------------------------------------------
// Side-effect: apply font-size CSS variable to the document
// ---------------------------------------------------------------------------

const FONT_SIZE_MAP: Record<FontSize, string> = {
  small: "11.5px",
  medium: "12.5px",
  large: "14px",
};

function applyFontSize(fontSize: FontSize): void {
  document.documentElement.style.setProperty("--editor-font-size", FONT_SIZE_MAP[fontSize]);
}

// ---------------------------------------------------------------------------
// Zustand store
// ---------------------------------------------------------------------------

export interface GeneralSettingsState {
  settings: GeneralSettings;
  actions: GeneralSettingsActions;
}

interface GeneralSettingsActions {
  updateSettings: (patch: Partial<GeneralSettings>) => void;
  resetSettings: () => void;
}

const useGeneralSettingsStoreBase = create<GeneralSettingsState>()(
  immer((set, get) => {
    const initial = loadSettings();

    // Apply font-size CSS variable immediately on store creation
    applyFontSize(initial.fontSize);

    return {
      settings: initial,

      actions: {
        updateSettings: (patch: Partial<GeneralSettings>) => {
          set((state) => {
            Object.assign(state.settings, patch);
          });
          const next = get().settings;
          persistSettings(next);
          applyFontSize(next.fontSize);
        },

        resetSettings: () => {
          set((state) => {
            state.settings = DEFAULT_GENERAL_SETTINGS;
          });
          persistSettings(DEFAULT_GENERAL_SETTINGS);
          applyFontSize(DEFAULT_GENERAL_SETTINGS.fontSize);
        },
      },
    };
  }),
);

export const useGeneralSettingsStore = createSelectors(useGeneralSettingsStoreBase);

// ---------------------------------------------------------------------------
// Backward-compatible hook — same shape the old Context-based hook returned
// ---------------------------------------------------------------------------

interface GeneralSettingsContextValue {
  settings: GeneralSettings;
  updateSettings: (patch: Partial<GeneralSettings>) => void;
  resetSettings: () => void;
}

/** @deprecated Use useGeneralSettingsStore directly */
export function useGeneralSettings(): GeneralSettingsContextValue {
  const settings = useGeneralSettingsStore((s) => s.settings);
  const actions = useGeneralSettingsStore((s) => s.actions);

  return {
    settings,
    updateSettings: actions.updateSettings,
    resetSettings: actions.resetSettings,
  };
}

/**
 * @deprecated Provider is no longer needed — GeneralSettings is now a Zustand store.
 * Kept as a pass-through wrapper for backward compatibility.
 */
export function GeneralSettingsProvider({ children }: { children: ReactNode }) {
  return children as ReactElement;
}
