// Station interior scene — R3F component rendering a complete 20x8x20 room per station.
// Conditionally mounted inside ObservatoryWorldScene when analyst enters a station interior.
// INTR-02, INTR-03, INTR-06
...
// --- Interior NPC component (idle bobbing only, no patrol) ---
...
interface InteriorNpcCrewProps {
  config: StationInteriorConfig;
}

function InteriorNpcCrew({ config }: InteriorNpcCrewProps) {
  return (
    <Instances limit={4}>
...
function HeroPropMarker({
  ...
  useFrame(({ clock }) => {
    ...
    if (isMissionTarget) {
      const pulse = 0.9 + Math.sin(clock.elapsedTime * 3) * 0.15;
  ...
  function handleClick() {
    ...
    const recipe: ObservatoryHeroPropRecipe = {
      ...
      // Minimal recipe fields — scene provides visual; store handles interaction
      kind: "hero-prop",
      glowColor,
  ...
  return (
    <group position={[wx, wy, wz]}>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0, 0]}>
        ...
        <meshStandardMaterial color={darkerWall} metalness={0.3} roughness={0.8} />
      </mesh>

      {/* Ceiling */}
      <mesh rotation={[Math.PI / 2, 0, 0]} position={[0, 8, 0]}>
        <planeGeometry args={[20, 20]} />
        <meshStandardMaterial color={darkerWall} metalness={0.2} roughness={0.9} />
      </mesh>
