...
/**
 * Safe version that returns null when the provider is not mounted.
 * Useful in components that may render outside the provider tree (e.g. tests).
 */
export function useHintSettingsSafe(): HintSettingsContextValue | null {
...
export function HintSettingsProvider({ children }: { children: ReactNode }) {
  ...
  const setShowHints = useCallback((show: boolean) => {
    setState((prev) => {
      const next = { ...prev, showHints: show };
      persistState(next);
      return next;
    });
  }, []);
  ...
  const updateHint = useCallback((id: HintId, patch: Partial<HintConfig>) => {
    setState((prev) => {
      const defaults = DEFAULT_HINTS[id];
      ...
      if (Object.keys(cleaned).length > 0) {
        newOverrides[id] = cleaned;
      } else {
        delete newOverrides[id];
      }

      const next = { ...prev, overrides: newOverrides };
      persistState(next);
      return next;
    });
  }, []);

  const resetHint = useCallback((id: HintId) => {
    setState((prev) => {
      const newOverrides = { ...prev.overrides };
      delete newOverrides[id];
      const next = { ...prev, overrides: newOverrides };
