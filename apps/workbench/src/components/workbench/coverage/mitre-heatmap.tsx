...
function intensityOpacity(ruleCount: number): number {
  ...
  if (ruleCount === 1) return 0.3;
  if (ruleCount === 2) return 0.5;
  return Math.min(0.8, 0.5 + ruleCount * 0.1);
}
      ...
      <div className="px-6 py-4 border-b border-[#2d3240] bg-[#0b0d13] shrink-0">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-mono" style={{ color: coverageColor }}>
                {coveredCount}/{totalCount} techniques ({coveragePercent}%)
              </span>
            </div>
        ...
        <div className="flex items-center gap-4 mt-3 text-[9px] font-mono">
          <span className="flex items-center gap-1.5">
            <span className="w-3 h-3 rounded-sm bg-[#0b0d13] border border-[#2d3240]" />
            <span className="text-[#6f7f9a]">No coverage</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span
              className="w-3 h-3 rounded-sm border border-[#7c9aef]/30"
              style={{ backgroundColor: "rgba(124,154,239,0.3)" }}
            />
            <span className="text-[#6f7f9a]">1 rule</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span
              className="w-3 h-3 rounded-sm border border-[#7c9aef]/50"
              style={{ backgroundColor: "rgba(124,154,239,0.5)" }}
            />
            <span className="text-[#6f7f9a]">2 rules</span>
          </span>
          <span className="flex items-center gap-1.5">
            <span
              className="w-3 h-3 rounded-sm border border-[#3dbf84]/60"
              style={{ backgroundColor: "rgba(61,191,132,0.7)" }}
            />
            <span className="text-[#6f7f9a]">3+ rules</span>
          </span>
        </div>
      </div>
            ...
            <div className="overflow-x-auto">
              <div className="inline-grid gap-0.5" style={{ gridTemplateColumns: `repeat(${MITRE_TACTICS.length}, minmax(90px, 1fr))` }}>
                ...
                {MITRE_TACTICS.map((tactic) => (
                  <div
                    key={tactic.id}
                    className="px-2 py-2 border-b border-[#2d3240]"
                  >
                    <span className="text-[8px] font-mono uppercase tracking-wider text-[#6f7f9a] leading-tight block">
                      {tactic.shortLabel}
                    </span>
                  </div>
                ))}
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          ruleCount === 0
                            ? "bg-[#0b0d13] border-[#2d3240]/50 hover:border-[#2d3240]"
                        ...
                        style={{
                          backgroundColor:
                            ruleCount > 0
                              ? `color-mix(in srgb, ${fillColor} ${Math.round(opacity * 100)}%, #0b0d13)`
                              : "#0b0d13",
                          borderColor:
                            ruleCount > 0
