...
export function AgentExplorer(_props: { windowId?: string }) {
  ...
  const filtered = useMemo(
    () => (search ? agents.filter((a) => a.agentId.toLowerCase().includes(search.toLowerCase())) : agents),
    [agents, search],
  );
  ...
  return (
    <div className="space-y-5" style={{ padding: 20, color: "var(--text)", overflow: "auto", height: "100%" }}>
      ...
      <div className="flex items-center gap-3">
        ...
        />
        <span className="font-mono" style={{ fontSize: 12, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--muted)" }}>
      ...
      {filtered.length === 0 && (
        <p className="font-mono" style={{ fontSize: 12, color: "rgba(154,167,181,0.4)", letterSpacing: "0.08em" }}>
      ...
      {selectedSession && sessionEvents.length > 0 && (
        <div>
          <div className="flex items-center gap-3" style={{ marginBottom: 8 }}>
            <span className="font-mono" style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: "0.1em", color: "var(--gold)" }}>
              ...
              <thead>
                <tr>
                  {["Time", "Action", "Target", "Guard", "Decision"].map((h) => (
                    <th key={h} className="font-mono px-4 py-2 text-[10px] uppercase" style={{ letterSpacing: "0.1em", color: "rgba(154,167,181,0.6)", fontWeight: 500 }}>
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {sessionEvents.map((e) => (
                  <tr key={e._id} className="hover-row">
                    ...
                    <td className="px-4 py-2">
                      {e.allowed === false ? <Stamp variant="blocked">BLOCKED</Stamp> :
                       e.allowed === true ? <Stamp variant="allowed">ALLOWED</Stamp> :
