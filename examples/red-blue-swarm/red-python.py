...
HUSHD_AGENT_ID = f"{SESSION_ID}:{AGENT_ID}" if SESSION_ID else AGENT_ID
...
def hushd_check(
    ...
    body: dict = {
        ...
        "target": target,
        # `session_id` in /api/v1/check is a hushd user-session concept and will be rejected
        # unless a real session exists. For this demo, correlate runs by embedding the run ID
        # into the agent_id.
        "agent_id": HUSHD_AGENT_ID,
    }
...
if _sdk_available:
    try:
        ...
        _ctx = GuardContext(cwd="/app", session_id=SESSION_ID, agent_id=HUSHD_AGENT_ID)
    except Exception as exc:
