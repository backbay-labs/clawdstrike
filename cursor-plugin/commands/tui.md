...
1. Detect the plugin root directory using this resolution order:
   ...
   - If `CURSOR_PLUGIN_ROOT` environment variable is set, use that
   - Otherwise, search ancestor directories from the current working directory for a `.cursor-plugin/plugin.json` file
   - Walk up from the cwd: check `$CWD/.cursor-plugin/plugin.json`, then `$CWD/../.cursor-plugin/plugin.json`, etc.
   - If found, the plugin root is the directory containing `.cursor-plugin/`
...
- Check that `CLAWDSTRIKE_PLUGIN_DIR` or `CURSOR_PLUGIN_ROOT` is set or that the plugin root could be found
