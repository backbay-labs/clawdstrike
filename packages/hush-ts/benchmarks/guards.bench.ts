...
function checkForbiddenPath(path: string, homeDir = "/Users/test"): boolean {
...
function checkSecrets(content: string): { found: boolean; type?: string } {
...
function checkEgress(domain: string): boolean {
...
function checkJailbreakHeuristic(input: string): { detected: boolean; pattern?: string } {
...
function checkJailbreakStatistical(input: string): { score: number; flags: string[] } {
  const flags: string[] = [];
  let score = 0;
...
async function main() {
  ...
  // ForbiddenPathGuard
  results.push(
    await benchmark("ForbiddenPath (safe)", () => checkForbiddenPath(safePath))
  );
  results.push(
    await benchmark("ForbiddenPath (blocked)", () => checkForbiddenPath(dangerousPath))
  );

  // SecretLeakGuard
  results.push(
    await benchmark("SecretLeak (clean)", () => checkSecrets(safeContent))
  );
  results.push(
    await benchmark("SecretLeak (detected)", () => checkSecrets(secretContent))
  );

  // EgressAllowlistGuard
  results.push(
    await benchmark("EgressAllowlist (allowed)", () => checkEgress(safeDomain))
  );
  results.push(
    await benchmark("EgressAllowlist (blocked)", () => checkEgress(blockedDomain))
  );

  // Jailbreak Heuristic
  results.push(
    await benchmark("Jailbreak Heuristic (safe)", () => checkJailbreakHeuristic(safeInput))
  );
  results.push(
    await benchmark("Jailbreak Heuristic (detected)", () =>
      checkJailbreakHeuristic(jailbreakInput)
    )
  );

  // Jailbreak Statistical
  results.push(
    await benchmark("Jailbreak Statistical (safe)", () =>
      checkJailbreakStatistical(safeInput)
    )
  );
  results.push(
    await benchmark("Jailbreak Statistical (suspicious)", () =>
      checkJailbreakStatistical(jailbreakInput)
    )
  );
