...
function makeFileReadEvent(path, id) {
  return {
...
function makeNetworkEvent(host, id) {
  return {
...
function makeCommandEvent(command, id) {
  return {
...
async function main() {
  ...
  results.push(
    await benchmark("File Read (allowed)", async () => {
      await engine.evaluate(makeFileReadEvent("/tmp/test.txt", counter++));
    })
  );

  results.push(
    await benchmark("File Read (blocked)", async () => {
      await engine.evaluate(makeFileReadEvent("~/.ssh/id_rsa", counter++));
    })
  );

  results.push(
    await benchmark("Network Egress (allowed)", async () => {
      await engine.evaluate(makeNetworkEvent("api.github.com", counter++));
    })
  );

  results.push(
    await benchmark("Network Egress (blocked)", async () => {
      await engine.evaluate(makeNetworkEvent("evil-exfil.com", counter++));
    })
  );

  results.push(
    await benchmark("Command Exec", async () => {
      await engine.evaluate(makeCommandEvent("ls -la", counter++));
    })
  );
  ...
  results.push(
    await benchmark(
      ...
      async () => {
        for (let i = 0; i < 10; i++) {
          await engine.evaluate(makeFileReadEvent(`/tmp/file${i}`, counter++));
        }
      },
      { iterations: 100 }
    )
  ...
  const avgOverhead =
    results.filter((r) => !r.name.includes("Sequential")).reduce((sum, r) => sum + r.avgMs, 0) /
    5;
