...
`infra/vendor/` now contains only three preserved path-deps:

- `nono/` -- first-party fork (Apache 2.0); we strip optional deps to keep
  the build tree small but do not alter functional code.
- `async-nats/` -- stock 0.40.0 (Apache 2.0); kept as a path-dep so the
  `[patch.crates-io]` block in the workspace `Cargo.toml` resolves
  deterministically across offline and online builds.
- `rustls-webpki/` -- ISC (with backport patches); carries local
  modifications for a security-advisory backport. Document the upstream
  issue ID and re-vendor when the advisory is published.

The rest of the offline-build mirror is regenerated by CI at run time via
`cargo vendor` and is NOT committed. Third-party licenses for those crates
remain unchanged in their crates.io packages.
