...
| --- | --- | --- | --- |
| Creation lifecycle | Separate `create` from `activate`. A thing is drafted first, then explicitly released into the world. | `Create hunt -> attach default spirit -> Configure Spirit -> release updated posture into dock/sidebar/3D workspace`. | Do not collapse this into a one-click icon picker or a long wizard. |
| Multimodal input | One shared state model can accept different input modes. | Support `Quick Configure`, `Thesis`, `Anchor Artifacts`, and later a light spatial mode if needed. | Do not copy Baia's exact `generate/draw/hybrid/upload` taxonomy just because it exists. |
...
| Pre-commit preview | Show ghost structure before the action is finalized. | Preview spirit posture in dock, sidebar wake, and 3D before `Apply Spirit`; preview semantic drop meaning before release. | Do not wait until after config or drop to reveal what the system thought was happening. |
...
| World release | The created thing should visibly enter the world. | `Configure Spirit` should culminate in a short draw-in, pulse, or transit into the active workspace scene when the spirit is affirmed or changed. | Do not reduce spirit configuration to a toast or silent state write. |
...
| Thresholded actions | Primary actions can stay dormant until there is enough signal. | Only light up deeper `Configure Spirit`, `Start Hunt from Staging`, or promoted semantic targets when the operator has provided enough context. | Do not surface advanced anticipatory actions at full strength from an empty state. |
...
| Memory model | Record activated objects, not every draft. | Persist configured spirits and major reconfiguration events as hunt history; drafts can remain ephemeral. | Do not copy Baia's localStorage grimoire or collectible inventory semantics. |
